...
function handleUrunForm() {
  let formData = new FormData(addUrunForm);
  ...
  if(formObj.id !== '') { // guncelle
    ...
    urun.aciklama = formObj.aciklama;
    urun.katagori = formObj.katagori;
    urun.img = formObj.img;
  } else { // yeni ekle
...
addUrunForm.addEventListener('submit', handleUrunForm);
...
function handleEditBtn(e) {
  ...
  let urunlerId = Number(this.dataset.urunid);
  let urun = urunler.find(x => x.id === urunlerId);
  ...
  document.querySelector('input[name="aciklama"]').value = urun.aciklama;
  document.querySelector('select[name="katagori"]').value = urun.katagori;
  document.querySelector('input[name="img"]').value = urun.img;
  modal.showModal();
}

function renderUrunler() {
  urunContainer.innerHTML = urunler.map(x => createUrunHtml(x)).join('');
  document.querySelectorAll('.UrunDeleteBtn')
    .forEach(x => x.addEventListener('click', handleDeleteBtn));
    document.querySelectorAll('.UrunEditBtn')
